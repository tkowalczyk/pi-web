...
async function main() {
	const cityId = parseInt(process.argv[2] || "0", 10);
	const streetId = parseInt(process.argv[3] || "0", 10);
	...
	const schedules = await db
		...
		.where(and(eq(waste_schedules.cityId, cityId), eq(waste_schedules.streetId, streetId)));
