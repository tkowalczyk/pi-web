...
export async function importer(dataDir: string) {
	...
	for (const fileData of loadedFiles) {
		for (const addr of fileData.data.addresses) {
			...
			if (!cityRecord) {
				[cityRecord] = await db.insert(cities).values({ name: addr.city }).returning();
				cityCount++;
			...
			if (isFirstTime) {
				...
				console.log(
					`    +${newStreetsInThisFile} additional streets for ${addr.city} (from ${fileData.filename})`,
				);
			}
		}
	}
	...
	for (const fileData of loadedFiles) {
		for (const [wasteTypeKey, wasteTypeName] of Object.entries(fileData.data.wasteTypes)) {
			if (!wasteTypeMap.has(wasteTypeKey)) {
				...
				if (!typeRecord) {
					[typeRecord] = await db.insert(waste_types).values({ name: wasteTypeName }).returning();
		...
		for (const addr of data.addresses) {
			...
			if (addr.streets.length === 0) {
				...
				const existingCityWide = await db
					.select()
					.from(streets)
					.where(and(eq(streets.name, cityWideStreetName), eq(streets.cityId, cityRecord.id)))
					.limit(1)
					.then((rows) => rows[0]);
			...
			console.log(
				`  Processing ${fileData.filename}: ${cityStreets.length} street(s) in ${cityRecord.name}...`,
			);
			...
			for (const [monthStr, monthSchedule] of Object.entries(data.wasteCollectionSchedule)) {
				...
				for (const [wasteTypeKey, days] of Object.entries(monthSchedule)) {
					...
					if (!wasteTypeId) {
						console.warn(
							`    Warning: Unknown waste type key "${wasteTypeKey}" in ${fileData.filename}`,
						);
						continue;
					}
					...
					for (const street of cityStreets) {
						...
						if (!exists) {
							await db.insert(waste_schedules).values({
								cityId: cityRecord.id,
								streetId: street.id,
								wasteTypeId,
								year,
