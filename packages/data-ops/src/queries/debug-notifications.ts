...
export async function debugUserNotifications(userIdOrEmail: string) {
	...
	const scheduleResults: Record<string, { tomorrow: string[]; today: string[] }> = {};
	...
	for (const pair of cityStreetPairs) {
		const schedules = await db
			...
			.where(
				and(eq(waste_schedules.cityId, pair.cityId), eq(waste_schedules.streetId, pair.streetId)),
			);
	...
	const phoneValid = user.phone ? /^\+48\d{9}$/.test(user.phone.replace(/\s/g, "")) : false;
