...
export async function createDefaultNotificationPreferences(userId: string, addressId: number) {
	...
	return await db.insert(notification_preferences).values(defaults).returning();
}
...
export async function updateNotificationPreference(
	...
	const db = getDb();
	await db.update(notification_preferences).set(data).where(eq(notification_preferences.id, id));
}
