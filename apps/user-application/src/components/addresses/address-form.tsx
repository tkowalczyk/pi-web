...
export function AddressForm() {
	...
	const mutation = useMutation({
		mutationFn: async (data: CreateAddressInput) => await createMyAddress({ data }),
		onSuccess: () => {
	...
	return (
		<form ref={formRef} onSubmit={handleSubmit} className="space-y-4 max-w-md">
			<div>
				...
				<p className="text-sm text-gray-600 mb-2">{t("address.cityDescription")}</p>
				<Select name="cityId" onValueChange={(val) => setCityId(Number(val))}>
					<SelectTrigger>
			...
			<div key={cityId}>
				...
				<Select name="streetId" disabled={!cityId}>
					<SelectTrigger>
