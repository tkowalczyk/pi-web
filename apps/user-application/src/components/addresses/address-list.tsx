...
export function AddressList({ addresses }: { addresses: AddressResponse[] }) {
	...
	if (addresses.length === 0) {
		return <div className="text-sm text-gray-600">{t("address.noAddresses")}</div>;
	}
					...
					{editingId === addr.id ? (
						...
						<div className="space-y-3">
							<div className="flex gap-2 items-start p-3 bg-red-50 rounded-lg border border-red-200">
								...
								<div className="text-sm">
									...
									<p className="text-red-600 mt-2">{t("address.deleteWarning")}</p>
								</div>
							</div>
						...
						<div className="flex items-start justify-between">
							<div>
								...
								<div className="text-sm text-gray-600 mt-1">{t("address.notificationTimes")}</div>
							</div>
							<div className="space-x-2">
								<Button variant="outline" size="sm" onClick={() => setEditingId(addr.id)}>
									{t("address.edit")}
								</Button>
								<Button variant="outline" size="sm" onClick={() => setDeleteConfirmId(addr.id)}>
									{t("address.delete")}
								</Button>
							</div>
						</div>
