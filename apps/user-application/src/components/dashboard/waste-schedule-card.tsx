		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>{t("waste_schedule.noUpcoming")}</CardDescription>
				</CardHeader>
			</Card>
		);
	}
	...
	return (
		<Card className="group hover:shadow-xl transition-all duration-300">
			<CardHeader>
				...
				<CardTitle>{t("waste_schedule.title")}</CardTitle>
				<CardDescription>{t("waste_schedule.upcomingDescription")}</CardDescription>
			</CardHeader>
			<CardContent>
						...
						return (
							<div
								...
								className={`flex items-center justify-between p-3 rounded-lg transition-colors ${
									isToday ? "bg-primary/10 border border-primary/20" : "bg-muted/50 hover:bg-muted"
								}`}
							>
...
function getWasteTypeColor(wasteType: string): string {
	...
	if (
		type.includes("metal") ||
		type.includes("tworzywa") ||
		type.includes("plastik") ||
